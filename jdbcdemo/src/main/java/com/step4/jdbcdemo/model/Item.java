	...
	public Object getProperty(String key) {
		...
		if (o instanceof ItemRelation) {
			...
			if (relation.reaRelationType.equals(RelationType.ONE_2_MANY)) {
				MetaRepository<? extends Item> repo = context.getBean(relation.referredModel + "Repository",
				...
				List<? extends Item> item = repo.findAllByColumnId(relation.referredColumn, relation.referredPK);
			...
			if (Item.class.isAssignableFrom(item_class)) {
				MetaRepository<? extends Item> repo = context.getBean(column.model + "Repository",
						MetaRepository.class);
