package com.nurkiewicz.jdbcrepository.derby.jdbcrepository;
...
public abstract class JdbcRepositoryCompoundPkTest extends AbstractIntegrationTest {
	...
	public void shouldStoreEntityWithCompoundPrimaryKey() throws Exception {
		...
		//then
		final BoardingPass found = repository.findById(pk("FOO-100", 42)).orElse(null);
	...
	public void shouldAllowStoringMultipleEntitiesDifferingByOnePrimaryKeyColumn() throws Exception {
		...
		//when
		final BoardingPass foundFlight = repository.findById(pk("BAR-100", 1)).orElse(null);
	...
	public void shouldAllowUpdatingByPrimaryKey() throws Exception {
		...
		final BoardingPass foundUpdated = repository.findById(pk("FOO-100", 2)).orElse(null);
	...
	public void shouldReturnNothingWhenFindingByListOfIdsButListEmpty() throws Exception {
		...
		//when
		final Iterable<BoardingPass> none = repository.findAllById(Collections.<Object[]>emptyList());
	...
	public void shouldSelectOneRecordById() throws Exception {
		...
		//when
		final List<BoardingPass> oneRecord = Lists.newArrayList(repository.findAllById(Arrays.<Object[]>asList(idOfSecond)));
	...
	public void shouldSelectMultipleRecordsById() throws Exception {
		...
		//when
		final List<BoardingPass> boardingPasses = Lists.newArrayList(repository.findAllById(Arrays.asList(idOfFirst, idOfThird)));
