...
public class SqlGenerator {
	...
	private String createWhereFromString(String where) {
		final StringBuilder whereClause = new StringBuilder(WHERE);
	...
	public String selectAllByColumn(TableDescription table , String columnName) {
		return SELECT + allColumnsClause + " " + FROM + table.getFromClause() + whereByIdsWithSingleIdColumn(1 , columnName) ;// whereByColumn(columnName);
	}
	...
	public String selectAllByWhereClause(TableDescription table , String whereClause) {
		return SELECT + allColumnsClause + " " + FROM + table.getFromClause() + createWhereFromString(whereClause);
	}
